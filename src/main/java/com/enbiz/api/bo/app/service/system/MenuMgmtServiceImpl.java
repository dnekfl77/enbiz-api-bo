...
import com.enbiz.api.bo.app.repository.main.system.StRtTgtBaseTrxMapper;
import com.enbiz.api.bo.app.repository.main.system.StRtInfoMapper;
...
public class MenuMgmtServiceImpl implements MenuMgmtService{
    ...
    public List<RtTargetBaseResponse> getMenuMgmtTreeList(RtTargetBaseRequest RtTargetBaseRequest) throws Exception {    	
    	int count = stRtTgtBaseMapper.getRightGroupMenuCheck(RtTargetBaseRequest);
    	if (count > 0) {
    		RtTargetBaseRequest.setRtSubGbCd("01");
    		return stRtTgtBaseMapper.getMenuMgmtTreeListForGroup(RtTargetBaseRequest);
    	} else {
    ...
    public void modifyMenuRight(Map<String, Object> list) throws Exception {
    	...
    	String checkedListTemp = list.get("checkedNodes").toString();
    	String nonCheckedListTemp = list.get("nonCheckedNodes").toString();
    	
    	String checkedListJson = checkedListTemp.replaceAll("&quot;", "\"");
    	String nonCheckedListJson = nonCheckedListTemp.replaceAll("&quot;", "\"");
    	
    	ObjectMapper mapper = new ObjectMapper();
    	ArrayList<Map<String, Object>> checkedList = mapper.readValue(checkedListJson, new TypeReference<ArrayList<Map<String, Object>>>(){});
    	ArrayList<Map<String, Object>> nonCheckedList = mapper.readValue(nonCheckedListJson, new TypeReference<ArrayList<Map<String, Object>>>(){});
    	
    	String sysGbCd = list.get("sysGbCd").toString();
    	String rtGrpNo = list.get("rtGrpNo").toString();
    	
    	StRtTgtBase stRtTgtBase = null;
    	StRtInfo stRtInfo = null;
    	for (int i=0; i<checkedList.size(); i++) {
    		stRtInfo = new StRtInfo();
    		stRtInfo.setRtGrpNo(rtGrpNo);
    		stRtInfo.setRtTgtSeq(checkedList.get(i).get("rtTgtSeq").toString());
    		stRtInfo.setRtSubGbCd("01");
    		stRtInfo.setSysRegId(loginId);
    		stRtInfo.setSysModId(loginId);
    		int count = stRtInfoMapper.getStRtInfo(stRtInfo);
    	...
    	for (int i=0; i<nonCheckedList.size(); i++) {
    		stRtInfo = new StRtInfo();
    		stRtInfo.setRtGrpNo(rtGrpNo);
    		stRtInfo.setRtTgtSeq(nonCheckedList.get(i).get("rtTgtSeq").toString());
    		stRtInfo.setRtSubGbCd("01");
    		stRtInfo.setSysRegId(loginId);
    		stRtInfo.setSysModId(loginId);
    		int count = stRtInfoMapper.getStRtInfo(stRtInfo);
